...
abstract class module{
		...
		public function __construct(&$c){
			try{
				...
				case  "mysql": $this->db=new PDO(
						"$c->driver:dbname=$c->database;host=$c->host;port=$c->port;",
											 $c->user,
											 $c->pass);
						break;
				case "sqlite":
						$this->db=new PDO("sqlite:$c->path/$c->database/$c->database" );
						break;
				default : throw new Exception("driver not specified");
				}

			}catch(exception $e){
				die("db-connection cannot be established.\n".$e->getMessage()."sqlite:".$c->path."/".$c->database
					);
			}
			...
			try{
				if(!$q=$this->db->query("SELECT max(id) FROM $c->table;"))
					throw new exception();
				else
	 ...
	 */
		 ...
		 */
		if(current($this->req)==="" && count($this->req)>1){
...
class sql_constructor{
	...
	public function __construct(&$stmt,&$_,&$cfg){
		...
		if($qobj->prepare)	foreach($qobj->prepare as $i => $key){
			 ...
			 */
			if($key=="table") $_["table"]=$this->cfg->table;
			if($key=="from"){
				if(!preg_match("/^from /",$_["from"])){
					if($_["from"])$_["from"] ="from ".$_["from"];
					else $_["from"]="from ".$this->cfg->table;
					}
				}
			...
			if($key=="columns" && !$_[$key]){
				...
				foreach($this->cfg->columns as $_key => $def){
						...
						//elseif($_["do"]=="insert")  //WTF sql-standard
						//	array_push($cols,'"'.$_key.'"'); 
				}
				$_[$key]=implode(",",$cols);
			...
			if($key=="values"){
				...
				foreach($this->cfg->columns as $_key => $def){
					...
					//	array_push($cols,"ifnull((SELECT max(id) FROM {$this->cfg->table})+1,1)");
					//else
					if($_[$_key])
